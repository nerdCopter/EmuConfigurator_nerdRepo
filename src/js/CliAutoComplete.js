...
CliAutoComplete._initTextcomplete = function() {
    ...
    $textarea.textcomplete('register', [
        ...
        strategy({ // "resource"
            ...
            search:  function(term, callback, match) { // eslint-disable-line no-unused-vars
                sendOnEnter = false;
        ...
        strategy({ // "resource index"
            ...
            replace: function(value) { // eslint-disable-line no-unused-vars
                if (this.savedTerm) {
                    self.openLater();
                    return '$1$3 ';
                }
    ...
    if (semver.gte(CONFIG.flightControllerVersion, "0.0.1")) {
        $textarea.textcomplete('register', [
            strategy({ // "resource show all", from BF 4.0.0 onwards
                ...
                search:  function(term, callback, matches) { // eslint-disable-line no-unused-vars
                    sendOnEnter = true;
